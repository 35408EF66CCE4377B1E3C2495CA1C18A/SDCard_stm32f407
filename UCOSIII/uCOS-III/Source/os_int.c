                   ...
                   OS_ERR       *p_err)
{
    ...
    if (p_err == (OS_ERR *)0) {
		...
		/* 
		* If the BSP init process takes to much time, and the OSInit() does not 
    * implement in time, the os_int_q chain table have not been initialized, the 
		* OSIntQInPtr should be point to a error address(like 0x00000000); 
		* The following assignment opration will cause the hard fault because
		* of the illegally access to an error address.
		*/
		if(OSIntQInPtr == 0x00000000)
		{
			return;
		}
